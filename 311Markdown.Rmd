...
corr_plot1 <-ggcorrplot(corr_matrix, hc.order = TRUE,  method = "square", type ="lower", outline.color = "white") + theme(axis.text.x = element_text(size = 10), axis.text.y = element_text(size = 10))
corr_plot1 +labs(title = "Correlation Matrix", subtitle = "311 Call Categories")
```
...
The data set contains 235 different neighbourhoods, so I'm only going to graph the top 5 neighbourhoods and categories for ease of reading in the next few graphs.
...
We can also see that neighbourhood livability complaints and missed household waste pickups dominate the number of calls to 311.
```{r, echo= FALSE}
wwplot + theme(axis.text.x = element_text(face = "bold", color = "#993333", size = 8, angle = 20, hjust = 1))
```

In William Whyte, the number of neighbourhood livability complaints is astronomical when compared to the other categories. Here are the top 5 categories of complaints, as well as graffiti. It's important to note that the scale of the y-axis for each category changes for ease of readability in each graph. 
```{r, echo=FALSE}
ggarrange(missed_household_waste_pickup_plot, neighbourhood_liveability_complaint_plot, nrow = 2)
ggarrange(potholes_plot, `snow_removal_-_roads_plot`, nrow = 2)
ggarrange(water_main_leak_plot, graffiti_plot, nrow = 2)
